...
convert_frame_to_ascii() {
    ...
    awk -v tlow="$threshold_low" -v thigh="$threshold_high" -v chars="$chars" '
    BEGIN {
        num_chars = length(chars)
        lum_range = thigh - tlow
    ...
    NR == 1 { next }
    {
        ...
        if (lum < tlow || lum > thigh) {
            row_text = row_text " "
            row_color[col_count] = ""
        } else {
            idx = int(((lum - tlow) * (num_chars - 1)) / lum_range)
